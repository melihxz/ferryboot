...
static void key_expansion(aes_context_t* context, const uint8_t* key, size_t key_len) {
    ...
    for (i = key_len / 4; i < 4 * (context->rounds + 1); i++) {
        ...
        if (i % (key_len / 4) == 0) {
            ...
            // XOR with round constant (simplified)
            temp ^= 0x01 << 24;
        }
...
int aes_decrypt(const aes_context_t* context, const uint8_t* ciphertext,
    ...
    // A real implementation would perform the full AES decryption algorithm
    
    // For demonstration, we'll just copy the ciphertext to plaintext
    memory_copy(plaintext, ciphertext, blocks * AES_BLOCK_SIZE);
